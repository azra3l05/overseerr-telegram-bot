...
from cache import cached, get_ttl
...
def overseerr_request(method: str, endpoint: str, **kwargs) -> requests.Response:
    ...
    # Default 30s timeout — prevents bot interactions from hanging if Seerr is slow/unresponsive.
    # Callers can still override by passing timeout=<n> explicitly.
...
# get_session_cookie()  # Lazy init instead
...
@cached(ttl=get_ttl("search"), key_prefix="search")
...
@cached(ttl=get_ttl("details"), key_prefix="details")
...
# Cache for Radarr/Sonarr root folder id->path mapping.
# Populated lazily on first lookup; refreshed on cache miss.
_root_folder_cache: Dict[str, Dict[int, str]] = {"movie": {}, "tv": {}}


def _get_root_folder_path(library_id: int, media_type: str) -> Optional[str]:
    """Translate a Radarr/Sonarr root folder numeric ID to its absolute path string.

    Seerr's /request API expects rootFolder as a string path, not a numeric ID.
    This helper queries the appropriate downstream service (Radarr for movies,
    Sonarr for TV) and caches the id->path map for the process lifetime.
    Returns None on lookup failure; caller should fall back to default routing.
    """
    cache_key = "movie" if media_type == "movie" else "tv"
    cache = _root_folder_cache[cache_key]

    if library_id in cache:
        return cache[library_id]

    # Cache miss — refresh from the service
    if media_type == "movie":
        url, key = RADARR_API_URL, RADARR_API_KEY
    else:
        url, key = SONARR_API_URL, SONARR_API_KEY

    if not url or not key:
        logger.warning(
            f"Cannot resolve root folder path for library_id={library_id}: "
            f"{media_type.upper()} API config missing"
        )
        return None
    ...
    try:
        ...
        logger.error(f"Failed to fetch {media_type} root folders: {e}")
        return None

    path = cache.get(library_id)
    if path is None:
        logger.warning(
            f"library_id={library_id} not found in {media_type} root folders "
            f"(known: {list(cache.keys())})"
        )
    return path


def request_media(media_id: int, media_type: str, seasons=None, library_id=None, tags=None):
    """Request media as telegrambot user with optional tags.

    NOTE: Seerr's /api/v1/request endpoint expects `rootFolder` as a string path,
    not `rootFolderId` as an integer. We translate library_id (Radarr/Sonarr root
    folder numeric ID) to the absolute path via the downstream service API before
    submitting. Prior implementation sent the wrong field name; Seerr silently
    dropped it and used its default root folder, causing non-default-library
    requests to misroute (typically to English).
    """
    data = {
    ...
    if library_id:
        root_folder_path = _get_root_folder_path(library_id, media_type)
        if root_folder_path:
            data["rootFolder"] = root_folder_path
        else:
            logger.warning(
                f"Could not resolve path for library_id={library_id} ({media_type}); "
                f"request will fall back to Seerr's default root folder"
            )
    if tags:
        data["tags"] = tags
...
def get_media_details(media_id, media_type, session_cookie=None):
    ...
    resp = requests.get(f"{OVERSEERR_API_URL}/{t}/{media_id}", headers=headers, timeout=15)
    resp.raise_for_status()
    return resp.json()


@cached(ttl=get_ttl("trending"), key_prefix="discover")
